...
class MainGame:
    ...
    def run(self):
        ...
        self.populate_increase_speed_event_triggers(self.game.controller.level_increase_timestamps(),
                                                    delay=delay * 1000)
                        ...
                        for side in sides:
                            CheckpointStar(
                                (self.game.all_sprites,
                                 self.game.left_fruits_sprites if side == 'Left' else self.game.right_fruits_sprites),
                                side_to_spawn=side,
                                **{k: v for k, v in spawn_params.items() if
                                   k != 'start_fps' and k != 'perfect_overlay_left' and k != 'perfect_overlay_right' and k != 'is_long' and k != 'health'}
                            )
                    else:
                        if is_double:
                            sides = ['Left', 'Right']
                        else:
                            sides = [fruit_dict['side_to_spawn']]
...
class Scoreboard:
    def __init__(self, player, game, player_info):
        ...
        self.health_text = Text(msg=f"Vida restante: {int(self.player.health)}%", position=(50, 350), color=WHITE,
                                font_size=40,
                                centered=False)
        ...
        if self.player.score == float(self.final_points.msg):
            self.time_needed = 0
        else:
            frames_needed = (self.player.score - float(self.final_points.msg)) / self.amount_to_add_initial_score
            self.time_needed = int(max(0, frames_needed / FPS) * 1000)

        self.event_trigger1 = pygame.event.custom_type()
        pygame.time.set_timer(self.event_trigger1, self.timer_for_final_score + self.time_needed, loops=1)

        self.event_trigger2 = pygame.event.custom_type()
        pygame.time.set_timer(self.event_trigger2, self.timer_for_final_score + self.time_needed - 1800, loops=1)
        ...
        while True:
            ...
            if elapsed_time > self.timer_for_final_score:
                ...
                if current_points < self.player.score:
                    ...
                    self.final_points.msg = f"{self.player.score:.0f}"
        ...
        try:
            ...
            if self.player.score > old_score:
                try:
                    ...
                    entries = rankingsDAO.get_rankings_by_music(self.game.song.song_name)
